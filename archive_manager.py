...
def rebuild_working_set(retention_days: int = 90) -> dict:
    """원장(archive/) + labels.db를 조인해 작업본(articles.csv)을 재생성한다.
    포함 기준: published_at이 최근 retention_days 이내 또는
    source가 "Player — "로 시작 (Player는 기간 무관 전량 유지).

    원장에서는 수집 8필드만 신뢰한다 — 렌즈 8필드(classified, category,
    event_tags, signal_type, sector, woomi_relevance, claude_rationale,
    korean_summary)는 항상 labels.db에서 조회해 덮어쓴다. labels.db에 없는
    article_id는 렌즈 필드를 빈 값(classified='False')으로 채운다.

    2026.09 응급 가드(원장 대신 기존 작업본의 classified=True 버전을 유지하던
    로직)는 이 함수에서 제거했다 — labels.db가 렌즈 산출물의 유일한
    source of truth가 되면서 가드가 대체됐다. 두 메커니즘을 동시에 두면
    어느 쪽이 우선인지 모호해진다.

    반환: {"total": N, "recent": N, "player": N, "partitions_read": N,
           "labels_found": N, "labels_missing": N}
    (recent·player는 겹칠 수 있어 합이 total과 다를 수 있다 — 진단용 원시 카운트)."""
    partitions = list_partitions()
    all_rows = read_archive(partitions)
    cutoff = datetime.now() - timedelta(days=retention_days)

    article_ids = [r.get("article_id") for r in all_rows if r.get("article_id")]
    ...
    for r in all_rows:
        ...
        if label is not None:
            ...
            merged["classified"] = "False"
            for f in label_store.LABEL_FIELDS:
                if f != "classified":
                    merged[f] = ""
            labels_missing += 1
        r = merged
    ...
    print(f"[INFO] labels 조회: 발견 {labels_found}건 / 누락 {labels_missing}건")
